...
public class SkipjackEngine implements BlockCipher {
    ...
    /* renamed from: g */
    private int m6277g(int i, int i2) {
        short s = i2 & 255;
        short[] sArr = ftable;
    ...
    /* renamed from: h */
    private int m6278h(int i, int i2) {
        short s = (i2 >> 8) & 255;
        short[] sArr = ftable;
        short s2 = (i2 & 255) ^ sArr[this.key3[i] ^ s];
        short s3 = s ^ sArr[this.key2[i] ^ s2];
        short s4 = s2 ^ sArr[this.key1[i] ^ s3];
        return ((sArr[this.key0[i] ^ s4] ^ s3) << 8) + s4;
    }
