...
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (i2 >= this.f5892V.length + this.mac.getMacSize()) {
            if (this.cipher == null) {
                i3 = (i2 - this.f5892V.length) - this.mac.getMacSize();
                ...
                for (int i5 = 0; i5 != i3; i5++) {
                    bArr3[i5] = (byte) (bArr[(this.f5892V.length + i) + i5] ^ bArr4[i5]);
                }
            } else {
                ...
                if (this.f5891IV != null) {
                    bufferedBlockCipher = this.cipher;
                    cipherParameters = new ParametersWithIV(new KeyParameter(bArr6), this.f5891IV);
                } else {
                    bufferedBlockCipher = this.cipher;
                    cipherParameters = new KeyParameter(bArr6);
                }
                bufferedBlockCipher.init(false, cipherParameters);
                bArr3 = new byte[this.cipher.getOutputSize((i2 - this.f5892V.length) - this.mac.getMacSize())];
                BufferedBlockCipher bufferedBlockCipher2 = this.cipher;
                byte[] bArr8 = this.f5892V;
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        System.arraycopy(bArr3, 0, bArr12, this.f5892V.length, i2);
        System.arraycopy(bArr10, 0, bArr12, this.f5892V.length + i2, macSize);
        return bArr12;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            this.f5891IV = parametersWithIV.getIV();
    ...
    public byte[] getLengthTag(byte[] bArr) {
        ...
        if (bArr != null) {
            Pack.longToBigEndian(((long) bArr.length) << 3, bArr2, 0);
        }
        return bArr2;
    }
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f5892V = generate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f5892V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
