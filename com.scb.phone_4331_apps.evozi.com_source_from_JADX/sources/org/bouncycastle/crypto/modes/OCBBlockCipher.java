...
public class OCBBlockCipher implements AEADBlockCipher {
    ...
    public byte[] getLSub(int i) {
        while (i >= this.f6000L.size()) {
            Vector vector = this.f6000L;
            vector.addElement(OCB_double((byte[]) vector.lastElement()));
        }
        return (byte[]) this.f6000L.elementAt(i);
    }
        ...
        if (bArr.length <= 15) {
            ...
            if (i == 0) {
                ...
                for (int i3 = 0; i3 < 16; i3++) {
                    ...
                    byte b = bArr3[i2];
                    i2++;
                    this.OffsetMAIN_0[i3] = (byte) (((bArr3[i2] & 255) >>> (8 - i)) | ((b & 255) << i));
                }
            }
            this.hashBlockPos = 0;
            this.mainBlockPos = 0;
