...
public class Tables64kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        if (this.f6011M == null) {
            this.f6011M = (int[][][]) Array.newInstance(Integer.TYPE, new int[]{16, 256, 4});
        } else if (Arrays.areEqual(this.f6010H, bArr)) {
            return;
        }
        this.f6010H = Arrays.clone(bArr);
        int i = 0;
        GCMUtil.asInts(bArr, this.f6011M[0][128]);
    ...
    public void multiplyH(byte[] bArr) {
        int[] iArr = new int[4];
        for (int i = 15; i >= 0; i--) {
            int[] iArr2 = this.f6011M[i][bArr[i] & 255];
