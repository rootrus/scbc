...
public class McElieceFujisakiCipher implements MessageEncryptor {
    ...
    /* renamed from: sr */
    private SecureRandom f6314sr;

    /* renamed from: t */
    private int f6315t;

    private void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        SecureRandom secureRandom = this.f6314sr;
        if (secureRandom == null) {
            secureRandom = new SecureRandom();
        }
        this.f6314sr = secureRandom;
        this.messDigest = Utils.getDigest(mcElieceCCA2PublicKeyParameters.getDigest());
        this.f6313n = mcElieceCCA2PublicKeyParameters.getN();
        this.f6312k = mcElieceCCA2PublicKeyParameters.getK();
        this.f6315t = mcElieceCCA2PublicKeyParameters.getT();
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            this.f6314sr = parametersWithRandom.getRandom();
    ...
    public void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f6313n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f6315t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        if (!this.forEncryption) {
            int i = (this.f6313n + 7) >> 3;
            int length = bArr.length - i;
            byte[][] split = ByteUtils.split(bArr, i);
            byte[] bArr2 = split[0];
            byte[] bArr3 = split[1];
            GF2Vector[] decryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f6313n, bArr2));
            ...
            if (Conversions.encode(this.f6313n, this.f6315t, bArr5).equals(gF2Vector)) {
                return bArr4;
            }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        if (this.forEncryption) {
            GF2Vector gF2Vector = new GF2Vector(this.f6312k, this.f6314sr);
            ...
            byte[] encoded2 = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, gF2Vector, Conversions.encode(this.f6313n, this.f6315t, bArr2)).getEncoded();
            DigestRandomGenerator digestRandomGenerator = new DigestRandomGenerator(new SHA1Digest());
            digestRandomGenerator.addSeedMaterial(encoded);
