...
public class RainbowSigner implements MessageSigner {
    ...
    /* renamed from: cf */
    private ComputeInField f6353cf = new ComputeInField();
    RainbowKeyParameters key;
    private SecureRandom random;
    int signableDocumentLength;

    /* renamed from: x */
    private short[] f6354x;

    private short[] initSign(Layer[] layerArr, short[] sArr) {
        short[] multiplyMatrix = this.f6353cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA1(), this.f6353cf.addVect(((RainbowPrivateKeyParameters) this.key).getB1(), sArr));
        for (int i = 0; i < layerArr[0].getVi(); i++) {
            this.f6354x[i] = (short) this.random.nextInt();
            short[] sArr2 = this.f6354x;
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        this.f6354x = new short[((RainbowPrivateKeyParameters) this.key).getInvA2().length];
        ...
        do {
            ...
            try {
                ...
                while (i < length) {
                    ...
                    short[] solveEquation = this.f6353cf.solveEquation(layers[i].plugInVinegars(this.f6354x), sArr);
                    if (solveEquation != null) {
                        for (int i4 = 0; i4 < solveEquation.length; i4++) {
                            this.f6354x[layers[i].getVi() + i4] = solveEquation[i4];
                        }
                        i++;
                    } else {
                ...
                short[] multiplyMatrix = this.f6353cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA2(), this.f6353cf.addVect(((RainbowPrivateKeyParameters) this.key).getB2(), this.f6354x));
