...
public class GMSSSigner implements MessageSigner {
    ...
    private void initSign() {
        ...
        if (gMSSPrivateKeyParameters.isUsed()) {
            ...
            this.ots = new WinternitzOTSignature(this.gmssRandom.nextSeed(bArr2), this.digestProvider.get(), this.gmssPS.getWinternitzParameter()[this.numLayer - 1]);
            ...
            while (true) {
                ...
                byte[][][] bArr3 = this.currentAuthPaths;
                int length = currentAuthPaths2[i3].length;
                int[] iArr = new int[2];
                iArr[1] = this.mdLength;
                iArr[0] = length;
                bArr3[i3] = (byte[][]) Array.newInstance(Byte.TYPE, iArr);
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        System.arraycopy(intToBytesLittleEndian, 0, bArr2, 0, intToBytesLittleEndian.length);
        System.arraycopy(signature, 0, bArr2, intToBytesLittleEndian.length, signature.length);
        System.arraycopy(concatenateArray, 0, bArr2, intToBytesLittleEndian.length + signature.length, concatenateArray.length);
        ...
        for (int i = (this.numLayer - 1) - 1; i >= 0; i--) {
            ...
            bArr3 = new byte[(intToBytesLittleEndian2.length + length2 + this.subtreeRootSig[i].length + concatenateArray2.length)];
            System.arraycopy(bArr4, 0, bArr3, 0, length2);
            System.arraycopy(intToBytesLittleEndian2, 0, bArr3, length2, intToBytesLittleEndian2.length);
            byte[][] bArr5 = this.subtreeRootSig;
            System.arraycopy(bArr5[i], 0, bArr3, intToBytesLittleEndian2.length + length2, bArr5[i].length);
            System.arraycopy(concatenateArray2, 0, bArr3, length2 + intToBytesLittleEndian2.length + this.subtreeRootSig[i].length, concatenateArray2.length);
        }
        ...
        for (int i2 = this.numLayer - 1; i2 >= 0; i2--) {
            ...
            for (int i5 = 0; i5 < bArr4.length; i5++) {
                ...
                if (length % 2 == 0) {
                    ...
                    System.arraycopy(bArr6, 0, bArr5, i8, i8);
                    length /= 2;
                } else {
                    System.arraycopy(bArr4[i5], 0, bArr5, 0, i6);
                    System.arraycopy(bArr, 0, bArr5, this.mdLength, bArr.length);
                    length = (length - 1) / 2;
                }
                this.messDigestTrees.update(bArr5, 0, i7);
