...
import p040o.setAutoSizeTextTypeUniformWithPresetSizes;
import p040o.setAutoSizeTextTypeWithDefaults;

public class TextLayer extends BaseLayer {
    private final setAutoSizeTextTypeWithDefaults<String> codePointCache = new setAutoSizeTextTypeWithDefaults<>();
    ...
    public void getBounds(RectF rectF2, Matrix matrix2, boolean z) {
        ...
        rectF2.set(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, (float) this.composition.getBounds().width(), (float) this.composition.getBounds().height());
    }
    ...
    private void drawTextGlyphs(DocumentData documentData, Matrix matrix2, Font font, Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            canvas2.translate(BitmapDescriptorFactory.HUE_RED, (((float) i) * dpScale) - ((((float) (size - 1)) * dpScale) / 2.0f));
    ...
    private void drawGlyphTextLine(String str, DocumentData documentData, Matrix matrix2, Font font, Canvas canvas, float f, float f2) {
        for (int i = 0; i < str.length(); i++) {
            FontCharacter MediaBrowserCompat$CustomActionResultReceiver = this.composition.getCharacters().MediaBrowserCompat$CustomActionResultReceiver(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
            if (MediaBrowserCompat$CustomActionResultReceiver != null) {
                drawCharacterAsGlyph(MediaBrowserCompat$CustomActionResultReceiver, matrix2, f2, documentData, canvas);
                float width = (float) MediaBrowserCompat$CustomActionResultReceiver.getWidth();
    ...
    private void drawTextWithFont(DocumentData documentData, Font font, Matrix matrix2, Canvas canvas) {
        ...
        if (typeface != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                canvas.translate(BitmapDescriptorFactory.HUE_RED, (((float) i) * dpScale) - ((((float) (size - 1)) * dpScale) / 2.0f));
    ...
    private float getTextLineWidthForGlyphs(String str, Font font, float f, float f2) {
        ...
        for (int i = 0; i < str.length(); i++) {
            FontCharacter MediaBrowserCompat$CustomActionResultReceiver = this.composition.getCharacters().MediaBrowserCompat$CustomActionResultReceiver(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
            if (MediaBrowserCompat$CustomActionResultReceiver != null) {
                f3 = (float) (((double) f3) + (MediaBrowserCompat$CustomActionResultReceiver.getWidth() * ((double) f) * ((double) Utils.dpScale()) * ((double) f2)));
            }
        }
        return f3;
    }
    ...
    static /* synthetic */ class C01343 {
        ...
        static {
            ...
            L_0x0028:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.airbnb.lottie.model.layer.TextLayer.C01343.<clinit>():void");
        }
    }

    private void applyJustification(DocumentData.Justification justification, Canvas canvas, float f) {
        int i = C01343.$SwitchMap$com$airbnb$lottie$model$DocumentData$Justification[justification.ordinal()];
        if (i == 2) {
    ...
    private String codePointToString(String str, int i) {
        ...
        setAutoSizeTextTypeWithDefaults<String> setautosizetexttypewithdefaults = this.codePointCache;
        ...
        if (setAutoSizeTextTypeUniformWithPresetSizes.MediaBrowserCompat$ItemReceiver(setautosizetexttypewithdefaults.write, setautosizetexttypewithdefaults.IconCompatParcelizer, j) >= 0) {
            return this.codePointCache.MediaBrowserCompat$ItemReceiver(j, null);
        }
        ...
        this.codePointCache.read(j, obj);
        return obj;
    }
