...
public class AnimatableTransformParser {
    private static JsonReader.Options ANIMATABLE_NAMES = JsonReader.Options.m33of("k");
    private static JsonReader.Options NAMES = JsonReader.Options.m33of("a", C8677p.f5438a, "s", "rz", "r", "o", "so", "eo", "sk", "sa");
    ...
    public static AnimatableTransform parse(JsonReader jsonReader, LottieComposition lottieComposition) throws IOException {
        ...
        while (jsonReader.hasNext()) {
            ...
            if (parseFloat.getKeyframes().isEmpty()) {
                ...
                Keyframe keyframe2 = new Keyframe(lottieComposition, Float.valueOf(BitmapDescriptorFactory.HUE_RED), Float.valueOf(BitmapDescriptorFactory.HUE_RED), (Interpolator) null, BitmapDescriptorFactory.HUE_RED, Float.valueOf(lottieComposition.getEndFrame()));
                parseFloat.getKeyframes().add(keyframe);
            } else if (((Keyframe) parseFloat.getKeyframes().get(0)).startValue == null) {
                z = false;
                parseFloat.getKeyframes().set(0, new Keyframe(lottieComposition, Float.valueOf(BitmapDescriptorFactory.HUE_RED), Float.valueOf(BitmapDescriptorFactory.HUE_RED), (Interpolator) null, BitmapDescriptorFactory.HUE_RED, Float.valueOf(lottieComposition.getEndFrame())));
                z2 = z;
    ...
    private static boolean isAnchorPointIdentity(AnimatablePathValue animatablePathValue) {
        return animatablePathValue == null || (animatablePathValue.isStatic() && ((PointF) animatablePathValue.getKeyframes().get(0).startValue).equals(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED));
    }

    private static boolean isPositionIdentity(AnimatableValue<PointF, PointF> animatableValue) {
        if (animatableValue == null || (!(animatableValue instanceof AnimatableSplitDimensionPathValue) && animatableValue.isStatic() && ((PointF) animatableValue.getKeyframes().get(0).startValue).equals(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED))) {
            return true;
        }
        return false;
    }
