...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f37pm = new PathMeasure();
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0088L.beginSection("StrokeContent#draw");
        if (Utils.hasZeroScaleAxis(matrix)) {
            C0088L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0088L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0088L.endSection("StrokeContent#buildPath");
                C0088L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0088L.endSection("StrokeContent#drawPath");
            }
        }
        C0088L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        float f;
        C0088L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0088L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f37pm.setPath(this.path, false);
        float length = this.f37pm.getLength();
        while (this.f37pm.nextContour()) {
            length += this.f37pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f37pm.setPath(this.trimPathPath, false);
            float length2 = this.f37pm.getLength();
            float f3 = 1.0f;
            if (floatValue3 > length) {
                ...
                if (f4 < f2 + length2 && f2 < f4) {
                    ...
                    Utils.applyTrimPathIfNeeded(this.trimPathPath, f, f3, BitmapDescriptorFactory.HUE_RED);
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C0088L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C0088L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0088L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0088L.endSection("StrokeContent#applyDashPattern");
            return;
        }
