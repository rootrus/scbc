                ...
                if (parseSettingsJson != null) {
                    ...
                    if (!SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(settingsCacheBehavior)) {
                        if (parseSettingsJson.isExpired(currentTimeMillis)) {
                            Logger.getLogger().mo8866d("Cached settings have expired.");
                            return null;
                        }
                    }
                    try {
                        Logger.getLogger().mo8866d("Returning cached settings.");
                        return parseSettingsJson;
                    } catch (Exception e) {
                        e = e;
                        settingsData = parseSettingsJson;
                        Logger.getLogger().mo8869e("Failed to get cached settings", e);
                        return settingsData;
                    }
                } else {
                    Logger.getLogger().mo8869e("Failed to parse cached settings data.", (Throwable) null);
                    return null;
                }
            } else {
                Logger.getLogger().mo8866d("No cached settings data found.");
                return null;
            }
        } catch (Exception e2) {
            e = e2;
