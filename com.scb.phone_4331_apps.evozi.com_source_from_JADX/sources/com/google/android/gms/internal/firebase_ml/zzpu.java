...
public final class zzpu {
    ...
    public final void zzb(zzpz zzpz) {
        if (zzpz.zzaxe.getBitmap() != null) {
            ...
            if (this.zzaxm.size() == 5 && elapsedRealtime - this.zzaxm.peekFirst().longValue() < 5000) {
                long j = this.zzaxn;
                if (j == -1 || elapsedRealtime - j >= TimeUnit.SECONDS.toMillis(5)) {
                    this.zzaxn = elapsedRealtime;
                    zzaoz.mo4183w("StreamingFormatChecker", "ML Kit has detected that you seem to pass camera frames to the detector as a Bitmap object. This is inefficient. Please use YUV_420_888 format for camera2 API or NV21 format for (legacy) camera API and directly pass down the byte array to ML Kit.");
                }
            }
        }
    }
