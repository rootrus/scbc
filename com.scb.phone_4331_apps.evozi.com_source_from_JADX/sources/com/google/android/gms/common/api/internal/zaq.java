...
final class zaq implements zabr {
    ...
    public static zaq zaa(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<Api.AnyClientKey<?>, Api.Client> map, ClientSettings clientSettings, Map<Api<?>, Boolean> map2, Api.AbstractClientBuilder<? extends zac, SignInOptions> abstractClientBuilder, ArrayList<zap> arrayList) {
        ...
        for (Map.Entry next : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                settextappearance.put((Api.AnyClientKey) next.getKey(), client2);
            } else {
                settextappearance2.put((Api.AnyClientKey) next.getKey(), client2);
            }
        }
        Preconditions.checkState(!settextappearance.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        setTextAppearance settextappearance3 = new setTextAppearance();
        setTextAppearance settextappearance4 = new setTextAppearance();
        for (Api next2 : map2.keySet()) {
            Api.AnyClientKey<?> clientKey = next2.getClientKey();
            if (settextappearance.containsKey(clientKey)) {
                settextappearance3.put(next2, map3.get(next2));
            } else if (settextappearance2.containsKey(clientKey)) {
                settextappearance4.put(next2, map3.get(next2));
            } else {
        ...
        while (i < size) {
            ...
            i++;
            zap zap = (zap) obj;
            if (settextappearance3.containsKey(zap.mApi)) {
                arrayList2.add(zap);
            } else if (settextappearance4.containsKey(zap.mApi)) {
                arrayList3.add(zap);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        return new zaq(context, zaaw, lock, looper, googleApiAvailabilityLight, settextappearance, settextappearance2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, settextappearance3, settextappearance4);
    }
    ...
    private zaq(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<Api.AnyClientKey<?>, Api.Client> map, Map<Api.AnyClientKey<?>, Api.Client> map2, ClientSettings clientSettings, Api.AbstractClientBuilder<? extends zac, SignInOptions> abstractClientBuilder, Api.Client client, ArrayList<zap> arrayList, ArrayList<zap> arrayList2, Map<Api<?>, Boolean> map3, Map<Api<?>, Boolean> map4) {
        ...
        for (Api.AnyClientKey<?> put : map2.keySet()) {
            settextappearance.put(put, this.zaei);
        }
        for (Api.AnyClientKey<?> put2 : map.keySet()) {
            settextappearance.put(put2, this.zaej);
        }
        this.zaek = Collections.unmodifiableMap(settextappearance);
    }
